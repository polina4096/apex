  ...
  S: Sample,
{
  ...
  ///
  pub fn new<D>(channels: u16, sample_rate: u32, data: D) -> ArcSamplesBuffer<S>
  where
    D: Into<Arc<[S]>>,
  {
    assert!(channels != 0);
...
impl<S> Source for ArcSamplesBuffer<S>
where
  S: Sample,
{
  #[inline]
...
impl<S> Iterator for ArcSamplesBuffer<S>
where
  S: Sample,
{
  type Item = S;
...
mod tests {
  ...
  mod try_seek {
    ...
    fn channel_order_stays_correct() {
      ...
      let mut buf = SamplesBuffer::new(CHANNELS, SAMPLE_RATE, (0 .. 2000i16).into_iter().collect::<Vec<_>>());
      buf.try_seek(Duration::from_secs(5)).unwrap();
      assert_eq!(buf.next(), Some(5i16 * SAMPLE_RATE as i16 * CHANNELS as i16));
